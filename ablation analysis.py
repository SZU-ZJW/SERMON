...
parser.add_argument('--checkpoint', type=str, default='./peter/without_retrival/',
...
parser.add_argument('--stage', type=str, default='train',
                    help='number of words to generate for each sample')
parser.add_argument('--model_path', type=str, default='/home/zjw/demo01/peter/without_retrival/model.pt',
                    help='number of words to generate for each sample')
...
def train(data,user_retrive_global,item_retrive_global):
    ...
    while True:
        ...
        c_loss = text_criterion(context_dis.view(-1, ntokens), seq[1:-1].reshape((-1,))) # 10.5156
        r_loss = rating_criterion(rating_p, rating)  # 29.3274
        t_loss = text_criterion(log_word_prob.view(-1, ntokens), seq[1:].reshape((-1,)))  # 10.4585
        cl_loss = model.contrast_loss(rating_vec,log_word_prob)  # 5.7613
        ...
        loss = args.text_reg * t_loss + args.context_reg * c_loss + args.rating_reg * r_loss + args.cl_reg * cl_loss  # 27.9918
        loss.backward()
    ...
    with torch.no_grad():
        while True:
            ...
            for idx in range(args.words):
                ...
                if idx == 0:
                    log_word_prob, log_context_dis, rating_p, _, _, _ = model(user, item, text, False,user_retrive_global=user_retrive_global,item_retrive_global=item_retrive_global)  # (batch_size, ntoken) vs. (batch_size, ntoken) vs. (batch_size,)
                    rating_predict.extend(rating_p.tolist())
                    context = predict(log_context_dis, topk=args.words)  # (batch_size, words)
                    context_predict.extend(context.tolist())
                else:
                    log_word_prob, _, _, _, _, _ = model(user, item, text, False, False, False,user_retrive_global=user_retrive_global,item_retrive_global=item_retrive_global)  # (batch_size, ntoken)
